...
function showLocation(following) {
...
function showFollowing (following) {
  return (<div key={following.login} className="follower-user clearfix">
            <div className='follower-img'>
              <a href={following.html_url} title={following.login}>  
            ...
            <div className="follower-details">
              <div className="follower-id clearfix">
                ...
                <a href={following.html_url} title={following.login}>
                  <div className="follower-login">{following.login}</div>
                </a>
              </div>
            {showBio(following)}
            <div className="location-company clearfix">
              {showCompany(following)}
              {showLocation(following)}
              </div>
             </div>
          </div>
         )
...
function Following(props) {
  useEffect(() => {
    if (props.followingIsLoading){
    get('users/Rpratik13/following' )
    ...
    .then(users => {
      users.map(user => {
        ...
        .then(response => response.json())
        .then(user => {
          props.addFollowing(user)
        })
        .catch(err => console.log(err));
      })
      props.setFollowingLoading(false);
    })
    .catch(err => console.log(err));
    }
  }, []);
  ...
  return (
          <div className = "followers-list">      
            {props.followingIsLoading && <div className="loading repo-loading" />}
            {!props.followingIsLoading && props.following.map(following => showFollowing(following))}        
          </div>
         );
}
...
function mapStateToProps(state) {
  return {
    ...
    followingIsLoading : state.following.followingIsLoading
  };
}
...
function mapDispatchToProps(dispatch) {
  return {
    addFollowing: user => {
      dispatch(followingActions.addFollowing(user));
    },

    setFollowingLoading: val => {
      dispatch(followingActions.setFollowingLoading(val));
    }
  }
}
